...
(function () {
  ...
  var css = ''
    ...
    + '#site-nav .nb-drawer summary .dic{width:24px;height:24px;flex:none;margin-right:8px;vertical-align:-6px}'
  ...
  GROUPS.forEach(function (g) {
    var hasActive = g.items.some(function (it) { return it[0] === path; });
    drawer += '<details' + (hasActive ? ' open' : '') + '><summary><img class="dic" src="/icons/' + CAT_ICON[g.label] + '.webp" width="24" height="24" alt="">' + esc(g.label) + ' <small>' + g.items.length + '개</small></summary><div class="dl">' + groupMenu(g) + '</div></details>';
  });
